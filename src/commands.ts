...
• World clock: "what time is it in Tokyo", "what's 9am PT in London", or just "what's 9am in Tokyo" (converts from YOUR time) → the time there or a zone conversion, instantly. Also "how long until 5pm" / "minutes until midnight" → a countdown in your timezone.
• Dates: "how many days until Christmas", "what day is July 4 2026", "how old if born 1990-05-06", "days between two dates", "3 business days from now", "is 2024 a leap year", "how many days in February" → exact calendar math, instantly.
...
Save recipes: "save <name>: <task>" stores a task you can re-run with /run <name>, or just "save that as <name>" right after I answer to keep the last task. /recipes lists them, /forget <name> removes one. Use a {slot} for a reusable recipe — "save track: price of {item}" then "/run track sneakers". Multiple slots take values by name or position — "save trip: {item} at {store}" then "/run trip item=milk store=HEB" (or "/run trip milk, HEB"). Chain steps with ">>" — "save plan: cheapest flight to {city} >> weather + top news there >> summarize" runs each step feeding the last, and "if <word>: <step>" only continues when the previous result contains that word. Interactive errands save too — "save jobs: on <board>, filter to remote react roles and list the newest with title + link" then "/run jobs" (or schedule it every morning) replays the whole browse-filter-list flow.

Bundle into a briefing: "define digest morning: weather, hn, btc" then /run morning (or schedule morning every morning) for one combined message. /digests lists them, /forget-digest <name> removes one. What moved since last time floats to the top with a ✦; add "(only if changed)" to the definition and a scheduled briefing stays quiet on a morning when nothing moved.
...
Pick from a list: when I send a numbered list (flights, listings, results), tap a 1/2/3 button OR just text "open the 2nd" / "#3" / "the last one" and I'll pull it up with its link.
...
Answer style: say "keep it brief" / "give me more detail" and I'll match that length going forward; "no emoji" (or "use emoji") toggles emoji. Stored in your profile so you don't repeat it — /profile shows your current style + how to change it.
...
Track anything about yourself: "log weight 182", "spent $14 on lunch", "log mood 7" — then "show my weight this month" (I'll chart it) or "how much did I spend on food this week" (I'll total it). Great for weight, spending, habits, sleep. Add "my logs" as a digest member ("define digest morning: weather, my logs") for a weekly recap of everything you've tracked in your briefing — or, without any digest, say "recap my logs weekly" and I'll send a "your week in numbers" every Monday ("stop log recaps" to turn it off).
