...
describe("runAgent dispatch", () => {
  ...
  it("get_holidays -> backend.getHolidays with the local today + country (holidays-tool)", async () => {
    const { b, hits } = recordingBackend();
    (b as { getHolidays?: (r: string, c: string | undefined, t: string) => Promise<string | null> }).getHolidays =
      async (r, c, t) => { hits.push(`getHolidays:${r}|${c ?? "-"}|${t}`); return "The next public holiday in GB is X."; };
    const llm = new ScriptLLM([
      { toolCall: { name: "get_holidays", args: { request: "next holiday", country: "UK" } } as ToolCall },
      { toolCall: { name: "reply", args: { text: "next up: X" } } as ToolCall },
    ]);
  ...
  it("text_stats counts/reverses/checks pure, reaches reply (text-stats)", async () => {
    const { b, hits } = recordingBackend();
    const llm = new ScriptLLM([
      { toolCall: { name: "text_stats", args: { request: "word count of the quick brown fox" } } as ToolCall },
      { toolCall: { name: "reply", args: { text: "4 words" } } as ToolCall },
    ]);
    const out = await runAgent("count the words", { llm, backend: b });
    expect(out.reply).toBe("4 words");
    expect(hits.filter((h) => h.startsWith("scrape") || h.startsWith("fetchJson"))).toHaveLength(0);
  });
  ...
  it("convert_base converts an integer between bases, pure, reaches reply (number-base-convert)", async () => {
    const { b, hits } = recordingBackend();
    const llm = new ScriptLLM([
      { toolCall: { name: "convert_base", args: { request: "255 in binary" } } as ToolCall },
      { toolCall: { name: "reply", args: { text: "0b11111111" } } as ToolCall },
    ]);
    const out = await runAgent("255 in binary", { llm, backend: b });
    expect(out.reply).toBe("0b11111111");
    expect(hits.filter((h) => h.startsWith("scrape") || h.startsWith("fetchJson"))).toHaveLength(0); // pure
  });

  it("convert_base with an unparseable request asks for number+base", async () => {
    const { b } = recordingBackend();
    const llm = new ScriptLLM([
      { toolCall: { name: "convert_base", args: { request: "some number thing" } } as ToolCall },
      { toolCall: { name: "reply", args: { text: "need a number + base" } } as ToolCall },
    ]);
    const out = await runAgent("base convert?", { llm, backend: b });
    expect(out.reply).toBe("need a number + base");
  });
  ...
  it("get_on_this_day -> backend.getOnThisDay with local today (on-this-day-tool)", async () => {
    const { b, hits } = recordingBackend();
    (b as { getOnThisDay?: (r: string, t: { month: number; day: number }) => Promise<string | null> }).getOnThisDay =
      async (r, t) => { hits.push(`getOnThisDay:${r}|${t.month}-${t.day}`); return "📅 On September 2 in history:\n• 2011: X"; };
    const llm = new ScriptLLM([
      { toolCall: { name: "get_on_this_day", args: { request: "what happened on this day" } } as ToolCall },
      { toolCall: { name: "reply", args: { text: "In 2011..." } } as ToolCall },
    ]);
    const out = await runAgent("on this day?", { llm, backend: b, nowMs: 1_756_800_000_000, tzOffsetMin: 0 }); // 2025-09-02
    expect(hits).toContain("getOnThisDay:what happened on this day|9-2");
    expect(out.reply).toBe("In 2011...");
  });

  it("get_on_this_day with no backend reports unavailable, still reaches reply", async () => {
    const { b } = recordingBackend();
    const llm = new ScriptLLM([
      { toolCall: { name: "get_on_this_day", args: { request: "on this day" } } as ToolCall },
      { toolCall: { name: "reply", args: { text: "used web_search" } } as ToolCall },
    ]);
    const out = await runAgent("history today", { llm, backend: b });
    expect(out.reply).toBe("used web_search");
  });
  ...
  it("get_holidays with no backend.getHolidays reports unavailable, still reaches reply", async () => {
    const { b } = recordingBackend(); // no getHolidays set
    const llm = new ScriptLLM([
      { toolCall: { name: "get_holidays", args: { request: "is today a holiday" } } as ToolCall },
      { toolCall: { name: "reply", args: { text: "used web_search" } } as ToolCall },
    ]);
    const out = await runAgent("holiday today?", { llm, backend: b });
    expect(out.reply).toBe("used web_search");
  });
