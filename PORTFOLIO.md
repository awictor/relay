...
From a clean checkout with anvil running locally (`ANVIL_BASE_URL`, default `:3000`) and Relay's
`.env` filled (`GEMINI_API_KEY`), five commands walk the whole story:

```bash
# in relay/
npm run preflight        # ONE GO/NO-GO across Relay + DataFaucet + anvil
npm run bench:latency    # real measured anvil latency (the THESIS.md numbers)
npm run e2e:faults       # induce real failures, watch it degrade gracefully
npm run status           # offline health: schedules/recipes/alerts + anvil capacity

# in ../mcp-forge/  (on the anvil-migration branch)
node scripts/merge-readiness.mjs   # READY-TO-MERGE verdict for the anvil→prod PR
```

Each is CI-safe: if anvil or a key is absent, the live ones SKIP (exit 0) rather than fail, so the
sequence runs anywhere and only exercises what the environment supports.
